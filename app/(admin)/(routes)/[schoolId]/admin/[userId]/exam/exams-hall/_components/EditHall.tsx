...
import { updateExamsHall } from "@/lib/actions/exam-hall.actions";
...
export function EditHall({ initialData }: { initialData: IExamHall }) {
    ...
    const hallId = initialData?._id as string;
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            await updateExamsHall(hallId, values, path);

            router.push(`/${schoolId}/admin/${userId}/exam/exams-hall`);
            form.reset();
            toast({
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormLabel>Enter Hall Name</FormLabel>
                                <FormControl>
